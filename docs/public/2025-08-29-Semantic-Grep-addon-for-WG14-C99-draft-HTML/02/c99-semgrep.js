...
(function () {
  ...
  function buildCollections() {
    ...
    // Wrap stray visible text nodes that are not inside a known block or heading.
    (function wrapStrays() {
      try {
        var tw = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null);
        var t;
        var withinSelector = CFG.blockSelector + ',' + CFG.headingSelector + ',#' + CFG.ids.root;
        while ((t = tw.nextNode())) {
          var val = t.nodeValue;
          if (!val || !val.trim()) continue;
          var par = t.parentElement;
          if (!par) continue;
          if (par.closest('#' + CFG.ids.root)) continue; // skip UI
          if (par.closest(withinSelector)) continue; // already inside a managed block/heading
          if (exclude.has(par)) continue;
          // create a lightweight wrapper so we can treat it as a block
          var span = document.createElement('span');
          span.className = CFG.classes.block + ' sg-stray';
          par.insertBefore(span, t);
          span.appendChild(t);
        }
      } catch (e) { /* best-effort */ }
    })();
    ...
    while ((node = walker.nextNode())) {
      ...
      if (node.closest('#' + CFG.ids.root)) continue; // skip UI subtree
