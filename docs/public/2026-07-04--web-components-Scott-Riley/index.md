...
**By:** AuthorScott Riley

**Source:** Piccalilli - https://piccalil.li/blog/framework-agnostic-design-systems-part-1/
...
export default class MyButton extends Elena(HTMLElement) {
   ...
   */
  render() {
    return html` <div class="my-button">${this.text}</div> `;
  }
}
MyButton.define();
```
...
renderButton(template) {
  return html` <button
        class="my-button" ${this.disabled ? "disabled" : nothing} > ${template} </button> `;
}
