...
export class LiquidSimulation {
    ...
    const cols = Math.max(
      ...
      Math.round(
        (portrait
          ...
          : this.config.simulation.baseCols) * this.qualityScale
      )
    );
    const rows = Math.max(
      28,
      Math.round(
        (portrait
          ...
          : this.config.simulation.baseRows) * this.qualityScale
      )
    );
  ...
  setQualityScale(scale) {
    const next = clamp(scale, 0.55, 1);
    if (Math.abs(next - this.qualityScale) < 0.001) return false;
  ...
  addImpulse(nx, ny, strength, radius = this.config.simulation.impactRadius) {
    ...
    for (let y = minY; y <= maxY; y += 1) {
      for (let x = minX; x <= maxX; x += 1) {
        ...
        const t = dist / r;
        const falloff = 1 - t;
        const depression = -this.config.simulation.impactCenterDepth * falloff * falloff;
        const ringDelta = t - this.config.simulation.impactRingPosition;
        const ringWidth = Math.max(0.0001, this.config.simulation.impactRingWidth);
        const ring =
          this.config.simulation.impactRingStrength *
          Math.exp(-(ringDelta * ringDelta) / (ringWidth * ringWidth));
        const index = y * this.cols + x;
        this.velocityField[index] += strength * (depression + ring);
      }
    }
