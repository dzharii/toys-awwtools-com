...
function main() {
  ...
  const accessibleLinks = document.getElementById("accessibleLinks");
  const muteToggle = document.getElementById("muteToggle");

  if (!(canvas instanceof HTMLCanvasElement)) {
    throw new Error("sceneCanvas not found");
  }
  if (!(pasteReceiver instanceof HTMLTextAreaElement)) {
  ...
  if (!(ambientHint instanceof HTMLDivElement)) {
    throw new Error("ambientHint not found");
  }
  if (!(accessibleLinks instanceof HTMLUListElement)) {
    throw new Error("accessibleLinks not found");
  }
