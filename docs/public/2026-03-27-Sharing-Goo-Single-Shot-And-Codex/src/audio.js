...
export class ReservoirAudio {
  ...
  tick(energy) {
    ...
    const targetBeatGain = 0.055 + this.activity * 0.02;
    ...
    if (this.beatOsc) {
      this.beatOsc.detune.cancelScheduledValues(now);
      this.beatOsc.detune.linearRampToValueAtTime(this.activity * 8, now + 0.18);
    }
    if (this.beatGain) {
      this.beatGain.gain.cancelScheduledValues(now);
      this.beatGain.gain.linearRampToValueAtTime(targetBeatGain, now + 0.18);
    }
  }
