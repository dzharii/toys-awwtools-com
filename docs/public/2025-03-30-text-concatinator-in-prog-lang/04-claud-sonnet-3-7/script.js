document.addEventListener('DOMContentLoaded', () => {
    ...
    const quoteEscapeInput = document.getElementById('quoteEscape');
    const maxWidthInput = document.getElementById('maxWidth');
    const concatOpInput = document.getElementById('concatOp');
    const finalSepInput = document.getElementById('finalSep');
    const resultingNewlineInput = document.getElementById('resultingNewline');
    ...
    // Escape quotes in the string using the configured escape sequence
    function escapeQuotes(str, quoteChar, escapeSequence) {
        if (quoteChar === '') return str;
        
        const regex = new RegExp(quoteChar, 'g');
        return str.replace(regex, escapeSequence);
    }

    // Format a single line of text with quotes, indentation, operator, and newline
    function formatSingleLine(line, indent, quote, escapeSequence, concatOp, resultingNewline, isLastLine, finalSep) {
        const escapedLine = escapeQuotes(line, quote, escapeSequence);
        // Add the resulting newline character at the end of each line
        let result = `${indent}${quote}${escapedLine}${resultingNewline}${quote}`;
    ...
    function splitLongLine(formattedLine, maxWidth, indent, quote, escapeSequence, resultingNewline, concatOp, isLastLine, finalSep) {
        const result = [];
        ...
        // Remove the resulting newline from the content since we'll add it back for each line
        if (content.endsWith(resultingNewline)) {
            content = content.substring(0, content.length - resultingNewline.length);
        }
        
        // Calculate how much content can fit per line
        // Account for the resulting newline in the calculation
        const maxContentPerLine = maxWidth - fixedLength - resultingNewline.length;
        ...
        while (content.length > 0) {
            ...
            if (isLastChunk) {
                lineEnd = resultingNewline + quote + (finalSep ? concatOp + finalSep : '');
            } else {
                lineEnd = resultingNewline + quote + concatOp;
            }
            
            result.push(fixedPrefix + escapeQuotes(chunk, quote, escapeSequence) + lineEnd);
        }
    ...
    function formatText() {
        ...
        const escapeSequence = quoteEscapeInput.value;
        const maxWidth = parseInt(maxWidthInput.value);
        const concatOp = concatOpInput.value;
        const finalSep = finalSepInput.value;
        const resultingNewline = resultingNewlineInput.value;
    ...
    quoteEscapeInput.addEventListener('input', debouncedFormatText);
    ...
    resultingNewlineInput.addEventListener('input', debouncedFormatText);
