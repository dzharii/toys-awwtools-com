...
export function createSymbolController(ctx) {
  ...
  function getSymbolContributionForFile(fileId) {
    const treeContribution =
      state.symbolRefs.contributions.treeByFile.get(fileId);
  ...
  function refreshEffectiveSymbolContribution(fileId) {
    ...
    if (effective)
      state.symbolRefs.contributions.effectiveByFile.set(fileId, effective);
  ...
  function addSymbolOccurrenceToIndex(bySymbol, contribution, occurrence) {
    ...
    refBucket.count += 1;
    refBucket.kindCounts.set(
      occurrence.kind || "reference",
      (refBucket.kindCounts.get(occurrence.kind || "reference") || 0) + 1,
    );

    const lineNumber = Number.isFinite(occurrence.lineNumber)
      ? Math.floor(occurrence.lineNumber)
      : 0;
    if (
      lineNumber > 0 &&
      !refBucket.lineNumbers.includes(lineNumber) &&
  ...
  function runSymbolIndexRebuildSlice(run) {
    ...
    try {
      while (performance.now() - start <= symbolRefsBuildSliceBudget) {
        if (run.stage === "scan") {
          ...
          collectWeakBridgeStats(
            run.files[run.fileIndex],
            run.weakFileMap,
            run.treeDefinitionSymbols,
          );
          run.fileIndex += 1;
        ...
        if (run.stage === "build") {
          if (run.fileIndex >= run.files.length) {
            ...
            state.symbolRefs.build.progress = {
              processed: run.files.length * 2,
              total: run.files.length * 2,
            };
          ...
          run.fileIndex += 1;
          state.symbolRefs.build.progress.processed =
            run.files.length + run.fileIndex;
          continue;
        }
    ...
    state.symbolRefs.build.pendingHandle = setTimeout(
      () => runSymbolIndexRebuildSlice(run),
      0,
    );
    updateControlBar();
  }
  ...
  function startSymbolIndexRebuild(reason = "baseline", onComplete = null) {
    ...
    if (!state.symbolRefs.enabled || state.phase !== "loaded") return;
    const files = [
      ...state.symbolRefs.contributions.effectiveByFile.values(),
  ...
  function runSymbolBaselineSlice(run) {
    ...
    try {
      while (run.fileIndex < run.files.length) {
        if (!run.file) {
          ...
          run.contribution = createNormalizedSymbolContribution(
            run.file,
            null,
            "heuristic",
          );
        ...
        while (run.lineIndex < run.lines.length) {
          ...
          const matches = extractHeuristicSymbolsFromLine(
            run.lines[run.lineIndex],
            {
              lineNumber,
          ...
          for (let i = 0; i < matches.length; i += 1) {
            ...
            if (item.role === "definition")
              run.contribution.definitions.push(item);
            else run.contribution.references.push(item);
          }
          run.lineIndex += 1;
        }

        state.symbolRefs.contributions.heuristicByFile.set(
          run.file.id,
          run.contribution,
        );
        const cached = state.treeSitter.cache[run.file.id]?.symbolRefs;
        if (cached) {
          const normalized = createNormalizedSymbolContribution(
            run.file,
            cached,
            "tree",
          );
          state.symbolRefs.contributions.treeByFile.set(
            run.file.id,
            normalized,
          );
        }
    ...
    if (
      run.fileIndex >= run.files.length &&
      run.id === state.symbolRefs.build.runId
    ) {
      state.symbolRefs.build.pendingHandle = null;
  ...
  function scheduleSymbolReferenceIncrementalUpdate(fileId) {
    if (!state.symbolRefs.enabled || !fileId || state.phase !== "loaded")
      return;
    const incremental = state.symbolRefs.incremental;
    ...
    incremental.debounceHandle = setTimeout(() => {
      incremental.debounceHandle = null;
      incremental.batchHandle = setTimeout(
        runSymbolReferenceIncrementalBatch,
        0,
      );
    }, symbolRefsIncrementalDebounceMs);
  }
  ...
  function runSymbolReferenceIncrementalBatch() {
    ...
    const pending = [...incremental.pendingFileIds].slice(
      0,
      symbolRefsIncrementalBatchSize,
    );
  ...
  function getSymbolReferenceStatusText() {
    ...
    const processed = Number.isFinite(progress.processed)
      ? progress.processed
      : 0;
  ...
  function buildReferenceLineDisplay(row, symbol, linesCache) {
    ...
    const preferredOccurrences = validOccurrences.some(
      (occ) => occ.source === "tree",
    )
    ...
    preferredOccurrences.forEach((occ) => {
      const line = Number.isFinite(occ.lineNumber)
        ? Math.floor(occ.lineNumber)
        : 0;
      if (line < 1 || firstOccurrenceByLine.has(line)) return;
      firstOccurrenceByLine.set(line, occ);
    });

    const validatedLines = [...firstOccurrenceByLine.keys()]
      .sort((a, b) => a - b)
  ...
  function buildSymbolReferencePanelContent(symbol, sourceFileId) {
    ...
    const defsText =
      totalDefs === 1 ? "1 definition" : `${totalDefs} definitions`;
    const refsText =
      totalRefs === 1 ? "1 reference" : `${totalRefs} references`;
    const summary = document.createElement("div");
    summary.className = "symbol-ref-summary";
    const partial =
      !state.symbolRefs.build.ready ||
      state.symbolRefs.build.partial ||
      state.symbolRefs.build.running;
    ...
    actions.appendChild(
      createSymbolReferenceActionButton("Copy symbol", "secondary tiny", () =>
        copyTextToClipboard(symbol),
      ),
    );
    actions.appendChild(
      createSymbolReferenceActionButton("Open search", "ghost tiny", () =>
        openSearchForSymbol(symbol),
      ),
    );
    ...
    if (!definitionRows.length) {
      ...
      definitionRows.forEach((row) => {
        ...
        defRow.addEventListener("click", () =>
          navigateToFileLine(row.fileId, row.lineNumber),
        );
    ...
    const referenceRows = entry?.referenceFiles
      ? [...entry.referenceFiles.values()]
          .map((bucket) => ({
            ...
            lineNumbers: (bucket.lineNumbers || [])
              .slice()
              .sort((a, b) => a - b),
          ...
          .sort(
            (a, b) =>
              b.count - a.count || a.sourcePath.localeCompare(b.sourcePath),
          )
      : [];
    ...
    if (!referenceRows.length) {
      ...
      const visibleRows = referenceRows.slice(
        0,
        symbolRefsMaxReferenceFilesShown,
      );
      visibleRows.forEach((row) => {
        ...
        path.textContent = row.sourcePath;
        const lineDisplay = buildReferenceLineDisplay(
          row,
          symbol,
          fileLinesCache,
        );
        ...
        lineDisplay.lines.forEach((line) => {
          ...
          if (
            Number.isFinite(occurrence?.startCol) &&
            occurrence.startCol > 0
          ) {
  ...
  function openSymbolReferencePanel(anchorEl, symbol, sourceFileId) {
    ...
    win.positionWindowNearElement(anchorEl, {
      resizeToFit: false,
      preferRight: true,
    });
    win.bumpActivity();
  ...
  function scheduleSymbolReferencePanelRefresh() {
    ...
    ui.refreshHandle = setTimeout(() => {
      ...
      if (
        !ui.activePanel ||
        !ui.activePanel.root?.isConnected ||
        !ui.activeSymbol
      )
        return;
      ui.activePanel.loadContent(
        buildSymbolReferencePanelContent(
          ui.activeSymbol,
          ui.activeSourceFileId,
        ),
      );
  ...
  function handleSymbolReferenceClick(event) {
    ...
    if (!symbol) return;
    openSymbolReferencePanel(
      event.target instanceof Element ? event.target : code,
      symbol,
      sourceFileId,
    );
  }
