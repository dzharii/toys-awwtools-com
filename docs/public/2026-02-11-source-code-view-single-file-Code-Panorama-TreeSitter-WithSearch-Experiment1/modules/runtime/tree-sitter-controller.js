...
export function createTreeSitterController(ctx) {
  ...
  function clampTreeSitterWindow(win) {
    ...
    const x = Math.min(
      Math.max(win.x ?? window.innerWidth - width - margin, margin),
      Math.max(margin, window.innerWidth - width - margin),
    );
    const y = Math.min(
      Math.max(win.y ?? Math.max(80, window.innerHeight * 0.2), margin),
      Math.max(margin, window.innerHeight - height - margin),
    );
    const chipY = Math.min(
      Math.max(win.chipY ?? window.innerHeight / 2 - 30, 60),
      Math.max(60, window.innerHeight - 160),
    );
  ...
  function updateTreeSitterPlacement() {
    ...
    persistTreeSitterState(state.treeSitter.window, {
  ...
  async function maybeInitTreeSitterRuntime() {
    ...
    try {
      const mod = await import(
        resolveAssetUrl(`${TREE_SITTER_ASSET_BASE}/web-tree-sitter.js`)
      );
      ts.Parser = mod.Parser;
      ts.Language = mod.Language;
      await ts.Parser.init({
        locateFile: () =>
          resolveAssetUrl(`${TREE_SITTER_ASSET_BASE}/web-tree-sitter.wasm`),
      });
  ...
  async function loadTreeSitterLanguage(kind) {
    ...
    try {
      const lang = await ts.Language.load(
        resolveAssetUrl(`${TREE_SITTER_ASSET_BASE}/${config.file}`),
      );
      ts.error = null;
      ts.progress.tsUnavailable = false;
  ...
  function refreshTreeSitterProgress() {
    ...
    progress.completedPhase1 = entries.filter(
      (entry) => entry.phase1Done,
    ).length;
    progress.completedPhase2 = entries.filter(
      (entry) => entry.phase2Done,
    ).length;
    progress.skipped = entries.filter(
      (entry) => entry.status === "skipped",
    ).length;
    progress.failed = entries.filter(
  ...
  function getTreeSitterQueueStatusText({ compact = false } = {}) {
    ...
    const failureText =
      progress.failed > 0 ? ` • errors ${progress.failed}` : "";
    ...
    return compact
      ? `Analysis ${progress.completedPhase1}/${total}`
      : `Analysis progress: ${progress.completedPhase1}/${total}`;
  }
  ...
  function runImmediateFileAnalysis(file) {
    ...
    const contribution = createNormalizedSymbolContribution(
      file,
      null,
      "heuristic",
    );
  ...
  function enqueueFileForBackgroundParsing(
    ...
    if (reason === "active") {
      ...
      const score = Number.isFinite(file.charCount)
        ? file.charCount
        : (file.textFull || file.text || "").length || 0;
      let inserted = false;
      for (let i = 0; i < queue.pendingIds.length; i += 1) {
        const current = getFileById(queue.pendingIds[i]);
        const currentScore = Number.isFinite(current?.charCount)
          ? current.charCount
          : current?.text?.length || Number.MAX_SAFE_INTEGER;
  ...
  function prioritizeFileInParseQueue(fileId) {
    ...
    if (queue.pendingSet.has(fileId)) {
      queue.pendingIds = [
        fileId,
        ...queue.pendingIds.filter((id) => id !== fileId),
      ];
      scheduleTreeSitterQueueSlice();
      scheduleTreeSitterStatusRefresh();
      return;
  ...
  function scheduleTreeSitterQueueSlice() {
    ...
    queue.handleType =
      typeof requestIdleCallback === "function" ? "idle" : "timeout";
  ...
  function getTreeSitterLanguage(file) {
    ...
    if (lower.endsWith(".json"))
      return state.settings.includeJson ? "json" : null;
  ...
  function renderTreeSitterPanel(statusText) {
    ...
    if (!status) {
      ...
      else if (!getTreeSitterLanguage(file))
        status =
          "Tree-sitter not available for this file. Lightweight analysis is available.";
      else if (!outline.length && !includes.length)
        status = "Background parsing pending for this file.";
    }
    if (els.tsStatus) {
