export function createReferencesController(ctx) {
  ...
  function detachReferenceDelegates() {
    ...
    els.fileContainer.removeEventListener(
      "keydown",
      handleReferenceTokenKeydown,
    );
  ...
  function clearAllReferenceDecorations() {
    const blocks = els.fileContainer
      ? els.fileContainer.querySelectorAll("pre code")
      : [];
  ...
  function ensureReferenceObserver() {
    ...
    state.refs.refsObserver = new IntersectionObserver(
      (entries) => {
  ...
  function observeAllReferenceSections() {
    ...
    const sections =
      els.fileContainer?.querySelectorAll?.(".file-section") || [];
  ...
  function collectTextNodesWithOffsets(code) {
    ...
    while (node) {
      if (
        node.nodeValue &&
        !(node.parentElement && node.parentElement.closest(".file-ref"))
      ) {
  ...
  function wrapReferenceTextSegment(
    ...
    span.dataset.refResolvedPaths = JSON.stringify(
      occurrence.resolvedPaths || [],
    );

    if (occurrence.resolutionStatus === "resolved") {
      span.title = occurrence.resolvedPaths?.length
        ? `In project: ${occurrence.resolvedPaths[0]}`
        : "In project";
  ...
  function decorateFileSectionReferences(fileId) {
    ...
    if (state.refs.build.running && !state.refs.index.bySource.has(fileId))
      return false;
  ...
  function pushReferenceOccurrence(file, lineNumber, candidate, resolution) {
    ...
    const targetKey =
      resolution.status === "resolved" && resolvedPaths.length === 1
        ? resolvedPaths[0]
    ...
    if (!target) {
      ...
      target.status = updateReferenceTargetStatus(
        target.status,
        resolution.status,
      );
      if (
        (!target.resolvedPaths || !target.resolvedPaths.length) &&
    ...
    target.totalCount += 1;
    target.countsBySource.set(
      sourceFileId,
      (target.countsBySource.get(sourceFileId) || 0) + 1,
    );
    if (!target.firstLineBySource.has(sourceFileId))
      target.firstLineBySource.set(sourceFileId, lineNumber);
      ...
      while (run.fileIndex < run.files.length) {
        ...
        while (run.lineIndex < run.lines.length) {
          if (performance.now() - start > REFS_BUILD_SLICE_BUDGET) {
            ...
            state.refs.build.pendingHandle = setTimeout(
              () => runReferenceIndexSlice(run),
              0,
            );
            return;
          }
  ...
  function buildReferencePanelContent(tokenData, sourceFileId) {
    ...
    if (effectiveStatus === "resolved") {
      ...
      const loaded =
        primaryPath && state.refs.pathToLoadedFileId.has(primaryPath);
      status.textContent = loaded
        ? `In project: ${primaryPath}`
        : `Exists but not loaded: ${primaryPath}`;
    ...
    const primaryFileId = primaryPath
      ? state.refs.pathToLoadedFileId.get(primaryPath)
      : "";
    if (primaryFileId) {
    ...
    if (!rows.length) {
      ...
      visibleRows.forEach((row) => {
        ...
        entryRow.addEventListener("click", () =>
          navigateToFileLine(row.fileId, row.line),
        );
  ...
  function handleReferenceTokenClick(event) {
    ...
    const sourceFileId =
      section?.dataset?.fileId || token.dataset.sourceFileId || "";
