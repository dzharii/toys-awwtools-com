const OUTLINE_CONFIGS = {
  ...
  scala: {
    ...
    map: (type) => {
      ...
      if (type === "method_definition" || type === "function_definition")
        return "function";
...
function extractNodeName(node) {
  ...
  for (const field of byField) {
    ...
    if (candidate && candidate.text) {
      const identifiers = candidate.descendantsOfType([
        "identifier",
        "type_identifier",
        "field_identifier",
      ]);
  ...
  const fallback = node.descendantsOfType([
    "identifier",
    "type_identifier",
    "field_identifier",
  ]);
...
export function buildOutlineModel(tree, file, lang) {
  ...
  nodes.forEach((node, idx) => {
    ...
    const name =
      lang === "json"
        ? node.child(0)?.text || extractNodeName(node)
