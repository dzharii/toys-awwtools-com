...
for (const item of cases) {
  assert.equal(countLinesFromText(item.text), item.expectedCount, `line count mismatch for ${JSON.stringify(item.text)}`);
  assert.deepEqual(buildLineStartOffsets(item.text), item.expectedOffsets, `offset mismatch for ${JSON.stringify(item.text)}`);
}
...
for (const lineNumber of [1, 2, 10, 77, 400, 700]) {
  ...
  assert.equal(lineNumberAtOffset(largeOffsets, offset), lineNumber, `round-trip mismatch for line ${lineNumber}`);
}
