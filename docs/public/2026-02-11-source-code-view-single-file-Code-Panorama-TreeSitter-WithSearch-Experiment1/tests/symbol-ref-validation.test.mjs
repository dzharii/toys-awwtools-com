...
assert.equal(findBoundedSymbolInLine("copyFileSourceHelper()", "copyFileSource"), -1);
...
assert.equal(
  isOccurrenceValidInLine("copyFileSource(file);", "copyFileSource", { startCol: 1, endCol: 15 }),
  true
);
assert.equal(
  isOccurrenceValidInLine("copyFileSourceHelper(file);", "copyFileSource", { startCol: 1, endCol: 15 }),
  false
);
assert.equal(
  isOccurrenceValidInLine("const x = copyFileSource(file);", "copyFileSource", { startCol: 0, endCol: 0 }),
  true
);
assert.equal(
  isOccurrenceValidInLine("const x = copyFileSourceHelper(file);", "copyFileSource", { startCol: 0, endCol: 0 }),
  false
);
