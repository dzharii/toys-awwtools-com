(() => {
  ...
  function analyzeAreaSelection(records, selectionRect) {
    const overlapping = records.filter((record) => rectIntersects(record.features.boundingRect, selectionRect));
  ...
  function createOverlayApp(windowObject = window) {
    ...
    function renderAreaPreview(areaRect) {
      ...
      const overlapping = state.scanRecords.filter((record) => rectsOverlap(record.features.boundingRect, areaRect)).slice(0, 28);
