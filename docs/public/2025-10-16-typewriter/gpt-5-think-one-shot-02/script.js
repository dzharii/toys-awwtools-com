(function () {
  ...
  function resizePaper() {
    ...
    const newHeight = Math.max(paper.scrollHeight, emToPx(9.5));
  ...
  function buildKeyboard() {
    ...
    // Paint servers and shadows INSIDE the SVG for reliability
    const defs = el('defs', {});
    ...
    rows.forEach((row, rowIdx) => {
      ...
      row.forEach((k) => {
        ...
        // **Attributes** (not only CSS) so no "all black" issue
        const rect = el('rect', {
          class: 'key',
          x: String(x),
          y: String(y),
        ...
        grp.addEventListener('pointerup', (ev) => { ev.preventDefault(); releaseVisual(k.code); });
        grp.addEventListener('pointerleave', () => releaseVisual(k.code));
  ...
  window.addEventListener('keydown', (e) => {
    ...
    // **Fix:** Do NOT steal focus from interactive controls (enables sound dropdown)
    if (isInteractive(e.target)) {
      return; // no sounds/highlights when operating controls
    }
