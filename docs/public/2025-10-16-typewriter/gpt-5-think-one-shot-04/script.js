(function () {
  ...
  // ---------------------------
  // Paper auto-grow with max-height + caret auto-scroll
  // ---------------------------
  ...
  function scrollCaretIntoView() {
    // If caret near bottom, ensure it remains visible; use selection range trick.
    const pos = paper.selectionStart ?? paper.value.length;
    const lineHeight = parseFloat(getComputedStyle(paper).lineHeight) || 24;
    // Estimate caret Y by counting newlines; cheap but effective for textareas
    const caretLine = (paper.value.slice(0, pos).match(/\n/g)?.length || 0) + 1;
    const caretY = (caretLine - 1) * lineHeight;
    if (caretY > paper.scrollTop + paper.clientHeight - lineHeight * 2) {
      paper.scrollTop = caretY - paper.clientHeight + lineHeight * 2;
    }
  }
  paper.addEventListener('input', () => { resizePaper(); scrollCaretIntoView(); });

  // Allow clicking anywhere in the paper container to focus the paper
  paperContainer.addEventListener('pointerdown', (e) => {
    if (e.target !== paper) {
      e.preventDefault();
  ...
  function buildKeyboard() {
    ...
    // Layout tuning to avoid crowding/overlap at right edge
    const paddingX = 20, paddingY = 20, gapX = 10, gapY = 14;
    const baseKeyW = 56, keyH = 54;
    let y = paddingY;

    rows.forEach((row, rowIdx) => {
      let x = paddingX + (rowIdx === 1 ? 12 : rowIdx === 2 ? 20 : rowIdx === 3 ? 28 : 0);
      ...
      row.forEach((k) => {
        ...
        // Prevent label overlap by squeezing long text a bit if needed
        if (k.label.length > 7) {
  ...
  function clearAllActive() { Array.from(activeKeys).forEach(releaseVisual); }
  ...
  // ---------------------------
  // Hardware keyboard handling (no focus stealing)
  // ---------------------------
  const codeToDisplayCode = new Set(rows.flat().map(k => k.code));

  function withinControls(el) { return el && controls.contains(el); }
  function isPaper(el) { return el === paper; }
  function isTypingContext(el) { return isPaper(el) || el === document.body || el === document.documentElement; }

  window.addEventListener('keydown', (e) => {
    ensureAudioContext();

    // Ignore when interacting with UI controls (e.g., dropdown)
  ...
  window.addEventListener('keyup', (e) => {
    if (withinControls(e.target) && !isPaper(e.target)) return;
    const code = e.code;
    if (codeToDisplayCode.has(code)) releaseVisual(code);
  });
  ...
  // Ensure audio can start with user gesture
